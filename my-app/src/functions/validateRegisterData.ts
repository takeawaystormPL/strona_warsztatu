...
export default function validateRegisterData(
  ...
  if ((data as RegisterData).email !== undefined) {
    ...
    if (!emailRegex.test((data as RegisterData).email))
      return { message: "Niepoprawny email", isValid: false };
  }
  const usernameRegex = /\b(?![#._,])[a-zA-Z0-9#._]/;
  if (!usernameRegex.test(data.username))
    return {
      message: "Nazwa użytkownika nie spełnia wymaganego formatu",
      isValid: false,
    };
  const passwordRegexes = [/.*[\W]\.*/, /[A-Z]/, /^\w{1,11}$/];
  const messages = [
    "Hasło nie zawiera znaku specjalnego",
    "Hasło nie zawiera dużych liter",
    "Hasło ma mniej niz 12 znaków",
  ];
  for (let i = 0; i < 2; i++) {
